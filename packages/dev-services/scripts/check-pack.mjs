...
const requiredFiles = [
	"README.md",
	"bin/lightfast-dev-services.mjs",
	"dist/public.js",
	"dist/public.d.ts",
	"dist/cli.js",
	"dist/cli.d.ts",
...
const binPath = normalizePackagePath(packageJson.bin?.["lightfast-dev-services"]);
const binFile = files.get(binPath);
if (!binFile) {
	errors.push("Missing lightfast-dev-services bin in package tarball.");
} else if ((binFile.mode & 0o111) === 0) {
	errors.push("lightfast-dev-services bin is not executable in package tarball.");
}
