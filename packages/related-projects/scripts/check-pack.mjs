...
const binPath = normalizePackagePath(packageJson.bin?.["portless-mfe"]);
const binFile = files.get(binPath);
if (!binFile) {
	errors.push("Missing portless-mfe bin in package tarball.");
} else if ((binFile.mode & 0o111) === 0) {
	errors.push("portless-mfe bin is not executable in package tarball.");
}
